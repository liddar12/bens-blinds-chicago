...
} from '/var/task/BensBlindsChicago/bens-blinds/.netlify/dist/run/handlers/request-context.cjs'
import { getTracer, withActiveSpan } from '/var/task/BensBlindsChicago/bens-blinds/.netlify/dist/run/handlers/tracer.cjs'

process.chdir('/var/task/BensBlindsChicago/bens-blinds')
  ...
  const handlerResponse = await runWithRequestContext(requestContext, () => {
    return withActiveSpan(tracer, 'Next.js Server Handler', async (span) => {
      span?.setAttributes({
        ...
        monorepo: true,
        cwd: '/var/task/BensBlindsChicago/bens-blinds',
      })
      if (!cachedHandler) {
        const { default: handler } = await import('/var/task/BensBlindsChicago/bens-blinds/.netlify/dist/run/handlers/server.js')
